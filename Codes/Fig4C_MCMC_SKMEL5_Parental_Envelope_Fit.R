...
s1 <- subset(data, data$conc==8 & data$Time <=300)
...
ggplot(data = data, aes(x=data$Time, y=data$nl2, col=CellLine)) +
  theme_bw() + geom_smooth(span=.18, aes(fill=CellLine), fill = "grey", alpha=0.4, data=data, 
                           method = "loess", size=.5, level=0.95)+ 
  scale_colour_manual(values=c("black")) + ylim(-1.5, 1.5) + xlim(0,300)+
