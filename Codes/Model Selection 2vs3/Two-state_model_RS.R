...
solvePop <- function(pars, times, Tot = 10000){
  derivs <- function(t, state, pars){
    with(as.list(c(state, pars)),{
      ...
      dB <- ( 0.000 - k_ba)*B + k_ab * A
      return(list(c(dA, dB), tot = A0+B0, l2=log2(A+B), nl2 = (log2(A+B)-log2(Tot))))
    })
  }
  state <- with(as.list(pars), c(A = Tot * A0, B = Tot * B0))
...
model2stRS <- modFit(p= c(pars[c("k_ab", "k_ba", "A0", "B0")]), f = Objective, 
               lower=0, upper=c(0.06, 0.06, 1, 1),
               hessian = T)
pars[c("k_ab", "k_ba","A0", "B0")] <- c(model2stRS$par)
summary(model2stRS)
# ===========================================================================================================
...
ggplot(data = data, aes(x=data$Time, y=data$nl2, col=CellLine)) +
  ...
  scale_colour_manual(values=c("black")) + ylim(-3, 1) + xlim(0,300)+
