...
# ===========================================================================================================
data <- read.csv("SKMEL5_Population_Level_Mean_Response.csv", header=T, sep=",")
s1 <- subset(data, data$conc==8 & data$Time <=350)
d_exp <- data.matrix(s1[,c("Time", "nl2")]);
colnames(d_exp) <- c("time", "nl2")
# Fake data to assign high cost in cost function
d_exp1 <- data.frame(time=seq(0,500), nl2=c(-1000.0));
d_exp1 <- data.matrix(d_exp1)
cell <- "SKMEL5_Parental"
...
chain1 <- load("MCMC1_SKMEL5_Parental_150000_Run.RData")
dim(MCMC1$pars)
...
plot(nl2~time, data=dn, type="n", ylim=c(-1,1), xlim=c(0,300), ylab="", xlab="")
