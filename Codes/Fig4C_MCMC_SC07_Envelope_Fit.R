...
ggplot(data = data, aes(x=data$Time, y=data$nl2, col=CellLine)) +
  theme_bw() + geom_smooth(span=.45, aes(fill=CellLine), fill = "grey", alpha=0.9, data=data, 
  method = "loess", size=.5, level=0.95)+ 
  scale_colour_manual(values=c("black")) + ylim(-1.5, 1.0) + xlim(0,300)+
  theme(legend.position="none") + 
  geom_line(aes(x=dn$time, y=dn$nl2), data=dn, col="blue", alpha=0.1)+ ggtitle("")+ labs(x="", y="") +
